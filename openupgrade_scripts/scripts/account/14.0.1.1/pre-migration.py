...
def add_move_id_field_account_bank_statement_line(env):
    ...
    openupgrade.logged_query(
        ...
        WHERE absl.move_name = am.name OR am.statement_line_id = absl.id
        """,
    )
    openupgrade.logged_query(
        env.cr,
        ...
        WHERE absl.move_id IS NULL AND absl.payment_ref = ap.communication
        """,
    )
