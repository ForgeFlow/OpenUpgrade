...
def map_order_line_state(cr):
    cr.execute("""
    UPDATE purchase_order_line l SET state = o.state FROM
    purchase_order o WHERE l.order_id = o.id
    """)
...
def pricelist_property(cr, env):
    ...
    partner = []
    currency = []
    for properties in property_rec:
        if properties.value_reference:
            product_pricelist = properties.value_reference
            res_partner = properties.res_id
