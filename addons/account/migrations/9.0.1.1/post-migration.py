...
def map_journal_state(cr):
    openupgrade.map_values(
        ...
        [('purchase_refund', 'purchase'), ('sale_refund', 'sale'),
         ('situation', 'general')],
        table='account_journal', write='sql')
    
def map_amount_type(cr):
    openupgrade.map_values(
        cr,
...
def account_templates(cr):
    ...
    cr.execute("""
    UPDATE account_chart_template SET company_id = r.id from res_company r
    """)
    ...
    for n in range(company_count):
        ...
        for m in range(chart_template_count):
            ...
            cr.execute("""
            INSERT INTO account_chart_template (property_account_receivable_id,
            property_account_payable_id, property_account_expense_categ_id,
            property_account_income_categ_id, property_account_expense_id,
            property_account_income_id, transfer_account_id, create_uid,
            write_uid, create_date, write_date, name, code_digits, visible,
            currency_id, complete_tax_set, account_root_id, tax_code_root_id,
    ...
    cr.execute("""
    SELECT COUNT(chart_template_id), chart_template_id FROM
    account_tax_template GROUP BY chart_template_id
    """)
    ...
    for x in range(len(chart_template)):
        ...
        # Assuming there are no duplicate chart template having same
        # name and bank_account_view_id
        cr.execute("""
        SELECT id, company_id FROM account_chart_template WHERE name =
        (SELECT name FROM account_chart_template WHERE id=%(x)s)
        AND bank_account_view_id = (SELECT bank_account_view_id FROM
        account_chart_template WHERE id=%(x)s) AND id <> %(x)s
        """ % {'x': template_id})
        other_chart_template = cr.dictfetchall()
        other_chart_templates.append(other_chart_template)

    cr.execute("""
    UPDATE account_tax_template t SET company_id = c.company_id FROM
    account_chart_template c WHERE t.chart_template_id = c.id
    """)
    ...
    for f, k in zip(range(len(tax_count)), range(len(tax_ids))):
        for i in range(tax_count[f]):
            ...
            for n in range(company_count):
                ...
                cr.execute("""
                INSERT INTO account_tax_template (name, chart_template_id,
                company_id, create_uid, write_uid, create_date, write_date,
                child_depend, include_base_amount, account_id,
                refund_account_id, active, price_include, description,
                python_applicable, python_compute, type_tax_use, type,
                applicable_type, amount, sequence, amount_type)
                SELECT name, %(a)s, %(b)s,
                create_uid, write_uid, create_date, write_date, child_depend,
                include_base_amount, account_id, refund_account_id, active,
                price_include, description, python_applicable, python_compute,
                type_tax_use, type, applicable_type, amount, sequence ,
                'percent'
...
#
    for n in range(company_count):
        ...
        for m in range(statement_template_count):
            ...
            cr.execute("""
            INSERT INTO account_operation_template (name, sequence, account_id,
            tax_id, company_id, amount_type, amount, second_amount_type,
            second_amount, label, create_uid, write_uid, create_date,
            write_date)
...
def parent_id_to_m2m(cr):
    ...
    for m in list_of_taxes:
        ...
        cr.execute("""
        SELECT id, company_id FROM account_tax_template WHERE name =
        (SELECT name FROM account_tax_template WHERE id=%(child)s)
        ...
        cr.execute("""
        SELECT id, company_id FROM account_tax_template WHERE name =
        (SELECT name FROM account_tax_template WHERE id=%(parent)s)
    ...
    for a in range(len(child_tax_ids)):
        for x, y in zip(child_tax_ids[a], parent_tax_ids[a]):
            ...
            (parent_tax, child_tax) VALUES (%(parent_id)s, %(child_id)s)
...
def cashbox(cr):
    ...
    for m in range(len(bank_statement)):
        ...
        cr.execute("""
        SELECT pieces, number_opening FROM account_cashbox_line WHERE
        number_opening IS NOT NULL AND number_opening != 0 AND
        bank_statement_id = %s
        """ % bank_statement_id)
        ...
        cr.execute("""
        INSERT INTO account_bank_statement_cashbox (create_date) VALUES (NULL)
        RETURNING id
        """)
        ...
        for x in opening_cashbox:
            ...
            VALUES (%(cash_id)s, %(opening_number)s, %(pieces)s)
            """ % {'opening_number': opening_number, 'pieces': pieces,
                   'cash_id': cashbox_id})
        ...
        cr.execute("""
        SELECT pieces, number_closing FROM account_cashbox_line WHERE
        number_closing IS NOT NULL AND bank_statement_id  = %s
        """ % bank_statement_id)
        ...
        for x in closing_cashbox:
            ...
            """ % {'closing_number': closing_number, 'pieces': pieces,
                   'cash_id': cashbox_id})
...
def migrate(cr, version):
    ...
    # If the close_method is 'none' set to 'False', otherwise set to 'True'
    cr.execute("""
    ...
    anglo_saxon_installed = openupgrade.\
        is_module_installed(cr, 'account_anglo_saxon')
    ...
    openupgrade.m2o_to_x2m(cr, registry['account.bank.statement.line'],
                           'account_bank_statement_line', 'journal_entry_ids',
