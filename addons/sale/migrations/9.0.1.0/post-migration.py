...
def set_invoice_policy(cr):
    # Used to generate the invoice, set it to 'order' which will create
    # the invoice as per ordered quantities.
...
def map_order_state(cr):
    ...
    openupgrade.map_values(
        ...
        [('waiting_date', 'sale'), ('progress', 'sale'), ('manual', 'sale'),
         ('shipping_except', 'sale'), ('invoice_except', 'sale')],
        table='sale_order')
...
def product_id_env(env):
    product = env['product.product'].create({'name': 'Service Product',
                                             'type': 'service'})
    env.cr.execute("""UPDATE sale_order_line SET product_id = %s WHERE
