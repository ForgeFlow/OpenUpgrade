...
def map_expense_state(cr):
    ...
    openupgrade.map_values(
                           ...
                           [('confirm', 'submit'), ('accepted', 'approve'),
                            ('done', 'post'), ('paid', 'done'),
                            ('cancelled', 'cancel')],
                           table='hr_expense')
...
def hr_expense(cr):
    ...
    """)
    # Counting one2many hr_expense_line and later
    # creating hr_expense record for it.
    cr.execute("""
    ...
    FROM hr_expense_line GROUP BY expense_id) as voila
    where consider is not null
    """)
