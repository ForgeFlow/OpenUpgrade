...
def _prepare_man_svl_vals(price_history_rec, previous_price, quantity, company, product):
    ...
    svl_vals = {
        ...
        "description": _("Product value manually modified (from %s to %s)") % (previous_price, price_history_rec["cost"]),
        "value": value,
        "quantity": 0.0,
    }
    return svl_vals
...
def get_stock_moves(env, company_id, product_id):
    env.cr.execute("""
        SELECT sm.id, sm.company_id, sm.product_id, sm.date, sm.product_qty, sm.reference,
          ...
          CASE WHEN sl.usage <> 'internal' AND sld.usage = 'internal' THEN 'in'
        ...
        WHERE sm.company_id = %s AND sm.product_id = %s
            AND state = 'done'
        ORDER BY sm.company_id, sm.product_id, sm.date, sm.id
    """, (company_id, product_id))
    return env.cr.dictfetchall()
...
def generate_stock_valuation_layer(env):
    ...
    for product in products:
        for company in companies:
            _logger.info("Product:{} {} Company:{}".format(product.id, product.name, company.name))
                ...
                if product.cost_method in ("average", "fifo") and not float_is_zero(
                    ...
                    while h_index < len(history_lines):
                        ...
                        if float_compare(price_history_rec["cost"], previous_price, precision_digits=precision_price):
                            ...
                            previous_price = price_history_rec["cost"]
                        h_index += 1
            # svl_vals_list = svl_in_vals_list + svl_man_vals_list + svl_out_vals_list
            # svl_vals_list = sorted(svl_vals_list, key=lambda k: (k["create_date"]))
            # _logger.info("To create {}".format(len(svl_vals_list)))
            # if svl_vals_list:
            #     query_insert(env.cr, "stock_valuation_layer", svl_vals_list)
    ...
    # env["stock.valuation.layer"].create(all_svl_list)


@openupgrade.migrate()
def migrate(env, version):
    # env.cr.execute("""CREATE INDEX stock_valuation_layer_stock_valuation_layer_id_index ON public.stock_valuation_layer USING btree (stock_valuation_layer_id);""")
    # env.cr.execute("""DELETE FROM stock_valuation_layer WHERE create_date < '2021-03-15';""")
    # env.cr.execute("""DROP INDEX IF EXISTS stock_valuation_layer_stock_valuation_layer_id_index;""")
